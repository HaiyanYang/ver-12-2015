...
for jp in jparts:
    ...
    jelfnm = next( j for j in range(jp, lenAll) if '*Elset, elset=fnm' in All_lines[j] )
    
    # read fnm elements of this part
    elsetfnm = []
    for je in range(jelfnm+1,lenAll):
        eline = All_lines[je]
        # break out of for loop if end of elset section is reached
        if('*' in eline):
            break
        # read the elem indices of this elset
        for t in eline.split(','):
            try:
                elsetfnm.append(int(t))
            except ValueError:
                pass
    ...
    for je in range(jelem+1,lenAll):
        ...
        # go to next loop if this elem is not a fnm elem
        if not (id in elsetfnm):
            continue 
...
for ja in jassemblies:
    ...
    # find the lines of *Instance, *Nset and *Elset predelam
    jinstances = [ j for j in range(ja,lenAll) if '*Instance' in All_lines[j] ]
    jnsets     = [ j for j in range(ja,lenAll) if '*Nset'     in All_lines[j] ]
    jpredelam  = [ j for j in range(ja,lenAll) if '*Elset, elset=predelam' in All_lines[j] ]
    if (len(jinstances)>1):
